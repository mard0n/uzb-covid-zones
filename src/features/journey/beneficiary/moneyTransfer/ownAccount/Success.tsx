...
const Success = () => {
  ...
  const { ownAccount } = React.useContext(StateContext);
  const dispatch = React.useContext(DispatchContext);
  const { beneficiaryDetails } = ownAccount;
  ...
  const onClickViewAll = () => {
    history.replace({
      pathname: RoutePath.BENIFICIARY_BILL_PAYMENT_LANDING,
    });
  };
            ...
            <Box>
              ...
              <Box mb={4}>
                <UnderlineText color="primary">
                  <H2>
                    {t(
                      `beneficiary.moneyTransfer.manage.within.confirmation.success.title`
                    )}
                  </H2>
                </UnderlineText>
              </Box>
              <Box mb={10}>
                <Body2>
                  {t(
                    `beneficiary.moneyTransfer.manage.within.confirmation.success.desc`
                  )}
                </Body2>
              </Box>
              <Grid container>
